...
begin {
       ## Custom Debug logic:
       [System.Threading.Thread]::CurrentThread.CurrentCulture = [System.Globalization.CultureInfo]"en"
       [System.Threading.Thread]::CurrentThread.CurrentUICulture = [System.Globalization.CultureInfo]"en"

       Set-Location -Path 'C:'
   
        if (-not (Test-Path $PreferenceFile -ErrorAction 0)) {
            [System.IO.FileInfo]$PreferenceFile = "$PSScriptRoot\CMPackager.prefs", "C:\Scripts\CMPackager\CMPackager.prefs" | Resolve-Path -ErrorAction SilentlyContinue | 
                Convert-Path | Select-Object -First 1
        }
        if (-not ( Test-Path [System.IO.DirectoryInfo]$RecipePath)) {
            "$PSScriptRoot\CMPackager.prefs" | test-path
            [System.IO.DirectoryInfo]$RecipePath = "$PSScriptRoot\Recipes", "C:\Scripts\CMPackager\Recipes" | Resolve-Path -ErrorAction SilentlyContinue | 
                Convert-Path | Select-Object -First 1
        }
}
process {
 
    $Global:ScriptVersion = "20.05.07.1"
    ...
    function Add-LogContent {
        ...
        $Message = @{
            InputObject = "$(Get-Date -Format G) - $Content"
            FilePath    = $LogPath
            Append       = if($Load -and (Get-Item $LogPath -ErrorAction SilentlyContinue).length -gt $MaxLogSize) { $false } else { $true }
        }
        Out-File @Message
    }
    ...
    function Invoke-VersionCheck {
        ...
        elseif ((-not (Get-CMApplication -Name "$ApplicationName $ApplicationSWVersion" -Fast | Filter-CMPApplicationFolder)) -and (-not ([System.String]::IsNullOrEmpty($ApplicationSWVersion)))) {
            $NewApp = $true			
    ...
    function Start-ApplicationDownload {
        ...
        foreach ($Download in $Recipe.ApplicationDef.Downloads.ChildNodes) {
            ...
            if ($NewApp) {
                ...
                Copy-Item -Path $DownloadFile -Destination $DestinationPath -Force | Out-Null
    ...
    function Invoke-ApplicationCreation {
        ...
        ## Set Variables
        $ApplicationName = $Recipe.ApplicationDef.Application.Name
        # $ApplicationDefaultLanguageId = if ($Recipe.ApplicationDef.Application.DefaultLanguageId) {$Recipe.ApplicationDef.Application.DefaultLanguageId} else { 9 }
        $ApplicationPublisher = $Recipe.ApplicationDef.Application.Publisher
        ...
        $ApplicationAutoInstall = if ([System.Convert]::ToBoolean($Recipe.ApplicationDef.Application.AutoInstall) -and $Recipe.ApplicationDef.DeploymentTypes.DeploymentType.InstallationBehaviorType -match 'System') { $true } else { $false }
        ...
        Write-Verbose "$ApplicationDisplayName, $ApplicationPublisher, $ApplicationAutoInstall, $ApplicationDisplaySupersedence, $ApplicationIsFeatured"

        # Reference: https://docs.microsoft.com/en-us/powershell/module/configurationmanager/new-cmapplication
        $ApplicationFullName = "$ApplicationName $ApplicationSWVersion"
        #  -DefaultLanguageId $ApplicationDefaultLanguageId
        $NewAppCommand = 'New-CMApplication -Name "$ApplicationFullName" -LocalizedName "$ApplicationDisplayName" -SoftwareVersion "$ApplicationSWVersion" -ReleaseDate $(Get-Date) -AutoInstall $ApplicationAutoInstall -DisplaySupersedenceInApplicationCatalog $ApplicationDisplaySupersedence -IsFeatured $ApplicationIsFeatured'
        $CmdSwitches = ''
        ...
        if (-not ([System.String]::IsNullOrEmpty($ApplicationIcon)))  {
            if (Test-Path "$Global:IconRepo\$ApplicationIcon") {
                ...
                $Global:SendEmail = $true; $Global:SendEmail | Out-Null
                $Global:EmailBody += "   - ERROR: Unable to find icon $ApplicationIcon, creating application without icon`n"
            }
        }
        ...
        try {
            ...
            $Recipe.ApplicationDef.Application.FullName = $ApplicationFullName
        }
        catch {
            $AppCreated = $false
            ...
            $Global:EmailBody += "ERROR: Creating Application Failed!: $ApplicationFullName`n"
            $Global:EmailBody += "ERROR: $ErrorMessage`n"
            $Global:EmailBody += "ERROR: $FullyQualified`n"
            $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"
        }

        # Apply categories if supplied. This was not available during application creation
        if ($AppCreated) {
            try {
                ...
                Add-LogContent "ERROR: $FullyQualified"
                Add-LogContent "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)"
                $Global:EmailBody += "ERROR: Setting Application Categories Failed!: $ApplicationFullName`n"
                $Global:EmailBody += "ERROR: $ErrorMessage`n"
                $Global:EmailBody += "ERROR: $FullyQualified`n"
                $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"
            }
        ...
        if ($AppCreated) {
            try {
                ...
                Add-LogContent "ERROR: $FullyQualified"
                Add-LogContent "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)"
                $Global:EmailBody += "ERROR: ERROR: Application Move Failed!: $ApplicationFullName`n"
                $Global:EmailBody += "ERROR: $ErrorMessage`n"
                $Global:EmailBody += "ERROR: $FullyQualified`n"
                $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"                
            }
    ...
    function Add-DetectionMethodClause {
        ...
        try {
            ...
            Add-LogContent "ERROR: $ErrorMessage"
            Add-LogContent "ERROR: $FullyQualified"
            $Global:EmailBody += "ERROR: Creating Detection Method Clause Failed!`n"
            $Global:EmailBody += "ERROR: $ErrorMessage`n"
            $Global:EmailBody += "ERROR: $FullyQualified`n"
            $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"
        }
    ...
    function Add-RequirementsRule {
        ...
        Push-Location
        Set-Location $Global:CMSite
        Write-Verbose "`"$ReqRuleType of $ReqRuleGlobalConditionName $ReqRuleOperator $ReqRuleValue`" is being added"
    ...
    function Add-DeploymentType {
        ...
        foreach ($DeploymentType in $Recipe.ApplicationDef.DeploymentTypes.ChildNodes) {
            ...
            Switch ($DepTypeInstallationType) {
                Script {
                    Write-Verbose "Script Deployment"
                    ...
                    try {
                        ...
                        Add-LogContent "ERROR: $ErrorMessage"
                        Add-LogContent "ERROR: $FullyQualified"
                        $Global:EmailBody += "ERROR: Creating Deployment Type Failed`n"
                        $Global:EmailBody += "ERROR: $ErrorMessage`n"
                        $Global:EmailBody += "ERROR: $FullyQualified`n"
                        $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"
                        $DepTypeReturn = $false
                    ...
                    if ($DepTypeAddDetectionMethods) {
                        ...
                        if ($DepTypeDetectionMethods.Count -eq 1) {
                            ...
                            try {
                                ...
                                Add-LogContent "ERROR: $ErrorMessage"
                                Add-LogContent "ERROR: $FullyQualified"
                                $Global:EmailBody += "ERROR: Adding Detection Method Failed`n"
                                $Global:EmailBody += "ERROR: $ErrorMessage`n"
                                $Global:EmailBody += "ERROR: $FullyQualified`n"
                                $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"
                ...
                MSI {
                    ...
                    try {
                        ...
                        Add-LogContent "ERROR: $ErrorMessage"
                        Add-LogContent "ERROR: $FullyQualified"
                        $Global:EmailBody += "ERROR: Adding MSI Deployment Type Failed`n"
                        $Global:EmailBody += "ERROR: $ErrorMessage`n"
                        $Global:EmailBody += "ERROR: $FullyQualified`n"
                        $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"
                    ...
                    if ($DepTypeAddDetectionMethods) {
                        ...
                        $DeploymentTypeParam = @{
                            ...
                            AddDetectionClause = $DepTypeDetectionMethods
                        }
                        if ($DepTypeDetectionMethods.Count -gt 1) {
                            $DeploymentTypeParam.Add('DetectionClauseConnector', $DepTypeDetectionClauseConnector)
                        }

                        # Add-LogContent "Set-CMMsiDeploymentType $($DeploymentTypeParam.GetEnumerator() | foreach { "-$($_.Name) $($_.Value.ToString())" })"
                        ...
                        try {
                            Set-CMMsiDeploymentType @DeploymentTypeParam
                        }
                        catch {
                            $ErrorMessage = $_.Exception.Message
                            $FullyQualified = $_.Exeption.FullyQualifiedErrorID
            ...
            if (-not [System.String]::IsNullOrEmpty($DeploymentType.RequirementsRules)) {
                ...
                foreach ($DepTypeReqRule in $DepTypeReqRules) {
                    ...
                    Write-Verbose "Add-RequirementsRule $addRequirementsRuleSplat"
    ...
    function Update-ApplicationLatest {
        param (
            $Recipe
        )
        # $ApplicationLatest = Update-ApplicationLatest -Recipe $ApplicationRecipe

        $ApplicationFullName = $Recipe.ApplicationDef.Application.FullName
        $ApplicationLatestName = $Recipe.ApplicationDef.Application.LatestName
        $ApplicationFolderPath = $Recipe.ApplicationDef.Application.FolderPath
        if (-not ([System.String]::IsNullOrEmpty($Recipe.ApplicationDef.Application.FilterFolderPath))) {
            $ApplicationFilterFolderPath = [System.Convert]::ToBoolean($ApplicationFilterFolderPath)
        } else { $ApplicationFilterFolderPath = $false }

        if([System.String]::IsNullOrEmpty($ApplicationLatestName)) {
            Add-LogContent "The latest application for $ApplicationFullName is not defined in Recipe"
            return $true
        }
        ...
        if(-not ($ApplicationLatest = Get-CMApplication -Name $ApplicationLatestName | Filter-CMPApplicationFolder)) {
            Add-LogContent "The latest application `"$ApplicationLatestName`" not found, you can create Latest Application manually. Skip"
            return $true
        }
        
        ## Update Latest Application
        $SourceApplication = Get-CMApplication -Name $ApplicationFullName | Filter-CMPApplicationFolder
        $SourceApplicationXML = [Microsoft.ConfigurationManagement.ApplicationManagement.Serialization.SccmSerializer]::DeserializeFromString($SourceApplication.SDMPackageXML,$True)
        $ApplicationLatestXML = [Microsoft.ConfigurationManagement.ApplicationManagement.Serialization.SccmSerializer]::DeserializeFromString($ApplicationLatest.SDMPackageXML,$True)

        Add-LogContent "Copy new deployment types version from `"$ApplicationFullName`" to `"$ApplicationLatestName`""

        #Clear All DT for Latest Application
        $ApplicationLatestXML.DeploymentTypes.Clear()

        foreach ($SourceDeploymentType in $SourceApplicationXML.DeploymentTypes) {
            ## $SourceDeploymentType = $SourceApplicationXML.DeploymentTypes[0]

            $NewDeploymentTypeTitle = "{0} - {1} - {2}" -f $($SourceApplication.LocalizedDisplayName), $SourceDeploymentType.Title, $SourceApplication.SoftwareVersion

            Add-LogContent "Copy DT: `"$($SourceApplication.LocalizedDisplayName)`" to `"$NewDeploymentTypeTitle`""

            $NewDeploymentType = $SourceDeploymentType.Copy()

            $NewDeploymentType.Title = $NewDeploymentTypeTitle

            $NewDeploymentType.ChangeId()

            if ($NewDeploymentType.GetType().Name -eq 'DeploymentType') {
                $ApplicationLatestXML.DeploymentTypes.Add($NewDeploymentType)
            }
        }
        
        $ApplicationLatestXML.SoftwareVersion = $SourceApplicationXML.SoftwareVersion
        $UpdatedXML = [Microsoft.ConfigurationManagement.ApplicationManagement.Serialization.SccmSerializer]::SerializeToString($ApplicationLatestXML, $True)
        $ApplicationLatest.SDMPackageXML = $UpdatedXML
        try { 
            Set-CMApplication -InputObject $ApplicationLatest
            $Global:EmailBody += "       - Updated Latest Application: $ApplicationLatestName`n"
            return $true
        } catch {
            $Global:EmailBody += "       - ERROR: Updated Latest Application: $ApplicationLatestName`n"
            return $false
        }
        # Pop-Location
    }
            ...
            if (-not ([string]::IsNullOrEmpty($Recipe.ApplicationDef.Distribution.DistributeToGroup))) {
                ...
                foreach ($DistributionGroup in $DistributionGroups) {
                    try {
                        ...
                        Add-LogContent "ERROR: $ErrorMessage"
                        Add-LogContent "ERROR: $FullyQualified"
                        $Global:EmailBody += "ERROR: Content Distribution Failed`n"
                        $Global:EmailBody += "ERROR: $ErrorMessage`n"
                        $Global:EmailBody += "ERROR: $FullyQualified`n"
                        $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"
            ...
            if ([System.Convert]::ToBoolean($Deployment.DeploySoftware)) {
                ...
                foreach ($DeploymentCollection in $DeploymentCollections) {
                    try {
                        ...
                        New-CMApplicationDeployment -CollectionName $DeploymentCollection @DeploymentSplat | Out-Null
                    }
                    catch {
                        $ErrorMessage = $_.Exception.Message
                        $FullyQualified = $_.Exeption.FullyQualifiedErrorID
                        ...
                        Add-LogContent "ERROR: $ErrorMessage"
                        Add-LogContent "ERROR: $FullyQualified"
                        $Global:EmailBody += "ERROR: Deployment Failed`n"
                        $Global:EmailBody += "ERROR: $ErrorMessage`n"
                        $Global:EmailBody += "ERROR: $FullyQualified`n"
                        $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"
    ...
    function Invoke-ApplicationSupersedence {
        ...
        Write-Verbose "Supersedence is $SupersedenceEnabled"
        if ($SupersedenceEnabled) {
            # Get the Previous Application Deployment Type
            Push-Location
            Set-Location $CMSite

            $Latest2Apps = Get-CMApplication -Name "$ApplicationName*" -Fast | Filter-CMPApplicationFolder | Where-Object {
                ($_.Manufacturer -eq $ApplicationPublisher) -and ($_.IsExpired -eq $false) -and ($_.IsSuperseded -eq $false) -and ($_.LocalizedDisplayName -ne $ApplicationLatestName)
            } | Sort-Object -Descending { ($_.SoftwareVersion -replace '[-|\s].+$') -as [version] }, DateCreated | Select-Object -First 2
            
            Write-Verbose $($Latest2Apps | Format-Table LocalizedDisplayName,SoftwareVersion | Out-String)

            Write-Verbose "Latest 2 apps = $($Latest2Apps.LocalizedDisplayName)"
            if ($Latest2Apps.Count -eq 2) {
                ...
                foreach ($DeploymentType in $NewAppDeploymentTypes) {
                    ...
                    Set-CMApplicationSupersedence -InputObject $NewApp -CurrentDeploymentType $DeploymentType `
                                                  -SupersededApplication $OldApp -OldDeploymentType $SupersededDeploymentType `
                                                  -IsUninstall $UninstallOldApp
                }
            }
            Pop-Location
        }
    ...
    function Invoke-ApplicationCleanup {
        ...
        Write-Verbose "Cleanup is $CleanupEnabled"
        if ($CleanupEnabled) {

            Push-Location
            Set-Location $CMSite

            Write-Verbose "Keeping $Keep superseded revisions of $ApplicationName"

            $Applications = Get-CMApplication -Name "$ApplicationName*" | Filter-CMPApplicationFolder | Where-Object IsSuperseded -eq $true | 
                Sort-Object { ($_.SoftwareVersion -replace '[-|\s].+$') -as [version] }, DateCreated

            if(($Applications.Count - $Keep) -gt 0) {
                $Applications = $Applications | Select-Object -First ($Applications.Count - $Keep)

                foreach ($Application in $Applications) {
                    # Get the content location and remove it
                    Write-Verbose "Cleaning up $($Application.LocalizedDisplayName)"
                    Pop-Location
    ...
    function Send-EmailMessage {
        ...
        $Global:EmailBody += "`n`nThis message was automatically generated`nOn: $($Env:COMPUTERNAME).$($Env:USERDNSDOMAIN)`nFrom: $($PSCommandPath)`nLog path: $($Global:LogPath)"
        try {
    ...
    # Filter for Application in ApplicationFolder (ObjectPath) when <FolderPath>[[string]$CMAppFolder]</FolderPath> + <FilterFolderPath>[bool]True</FilterFolderPath>
    filter Filter-CMPApplicationFolder {
        [CmdletBinding()]
        param (
            [Parameter(Mandatory = $false)]
            [string] $CMAppFolder = $Global:ApplicationFolderPath
            
            ,[Parameter(Mandatory = $true, ValueFromPipeline = $true)]
            [object[]] $InputObject
    
            ,[Parameter(Mandatory = $false)]
            [bool] $FilterAppFolder = $Global:ApplicationFilterFolderPath
        )
    
        begin {
            Write-Verbose "CMAppFolder       = $CMAppFolder"
            Write-Verbose "InputObject.Count = $($InputObject.Count)"
            Write-Verbose "FilterAppFolder   = $FilterAppFolder"

            if($FilterAppFolder -and $CMAppFolder) {
                [int[]]$AppInFolder = Invoke-CMWmiQuery -Option Fast -Query ("SELECT CI_ID FROM SMS_ApplicationLatest WHERE ObjectPath = '/{0}' OR ObjectPath like '/{0}/%'" -f $CMAppFolder) | Select-Object -ExpandProperty CI_ID
                Write-Verbose "AppInFolder.Count = $($AppInFolder.Count)"
            }
        }
        ...
        process {
            ...
            elseif ($AppInFolder -contains $InputObject.CI_ID) {
                Write-Verbose "LocalizedDisplayName = $($InputObject.LocalizedDisplayName) in folder or fubfolder $CMAppFolder"
                $InputObject
            }
        }
    
        end {
            Write-Verbose "End Filter by folder"
        }
    }
	...
	reg add "HKCU\Software\Microsoft\Internet Explorer\Main" /t REG_DWORD /v DisableFirstRunCustomize /f /d 1 | Out-Null
    ...
    foreach ($Recipe in $RecipeList) {
        ...
        $ApplicationLatest = $false

        try {
            ## Import Recipe
            Add-LogContent "Importing Content for $Recipe"
            Write-Verbose "Begin Processing: $Recipe"
            [xml]$ApplicationRecipe = Get-Content $(Join-Path -Path $RecipePath -ChildPath $Recipe)

            if(-not ([System.String]::IsNullOrEmpty($ApplicationRecipe.ApplicationDef.Application.FolderPath))) {
                $Global:ApplicationFolderPath = $ApplicationRecipe.ApplicationDef.Application.FolderPath
            } else {$Global:ApplicationFolderPath = $null}
            if (-not ([System.String]::IsNullOrEmpty($ApplicationRecipe.ApplicationDef.Application.FilterFolderPath))) {
                $Global:ApplicationFilterFolderPath = [System.Convert]::ToBoolean($ApplicationRecipe.ApplicationDef.Application.FilterFolderPath)
            } else { $Global:ApplicationFilterFolderPath = $false}
            ...
            if ($Download) {
                Write-Verbose "Application Creation"
            ...
            if ($ApplicationCreation) {
                Write-Verbose "Application Deployment Type Creation"
            ...
            if ($DeploymentTypeCreation) {
                Write-Verbose "Application Distribution"
            ...
            if ($ApplicationDistribution) {
                Write-Verbose "Application Supersedence"
            ...
            if ($ApplicationSupersedence) {
                Write-Verbose "Application Deployment"
                $ApplicationDeployment = Invoke-ApplicationDeployment -Recipe $ApplicationRecipe
                Add-LogContent "Completed Processing Application Deployment of $Recipe - $ApplicationDeployment"
            }
            if ($ApplicationDeployment) {
                Write-Verbose "Application Cleanup"
                $ApplicationCleanup = Invoke-ApplicationCleanup -Recipe $ApplicationRecipe
                Add-LogContent "Completed Application Cleanup Processing of $Recipe - $ApplicationCleanup"
            }
            if ($ApplicationDeployment) {
                Write-Verbose "Application Latest"
                $ApplicationLatest = Update-ApplicationLatest -Recipe $ApplicationRecipe
                Set-Location c:
                Add-LogContent "Completed Update Latest of $Recipe - $ApplicationLatest"
            }
            ...
            Add-LogContent "ERROR: Processing ${Recipe}:"
            Add-LogContent "ERROR: $ErrorMessage"
            Add-LogContent "ERROR: $($ErrorMessage.ScriptStackTrace)"
            Add-LogContent "ERROR: $FullyQualified"

            $Global:EmailBody += "ERROR: Processing ${Recipe}:`n"
            $Global:EmailBody += "ERROR: $ErrorMessage`n"
            $Global:EmailBody += "ERROR: $($ErrorMessage.ScriptStackTrace)`n"
            $Global:EmailBody += "ERROR: $FullyQualified`n"
            $Global:EmailBody += "ERROR: $($_.CategoryInfo.Category): $($_.CategoryInfo.Reason)`n"
        }
    }
